 ...
 */
{%- endif %}
@JsonTypeInfo(use = JsonTypeInfo.Id.NAME, property = "{{schema.discriminator()}}")
@JsonSubTypes({
{%- for schemaName in schema._json['x-java-sub-classes'] %}
  @JsonSubTypes.Type(value = {{schemaName  | camelCase | upperFirst}}.class, name = "{{schemaName | camelCase | upperFirst}}"){% if not loop.last %},{% endif %}
{%- endfor %}
})
...
{# collect all properties of the schema also the ones from allOf #}
{% set schemaCollectedProps = schema | collectProperties %}

{% if schema.description() or schema.examples() %}/**{% for line in schema.description() | splitByLines %}
 * {{ line | safe}}{% endfor %}{% if schema.examples() %}
 * Examples: {{schema.examples() | examplesToString | safe}}{% endif %}
 */{% endif %}
public class {{schemaName | camelCase | upperFirst}} {{schema | getImplementsClass}} {
    {% for propName, prop in schemaCollectedProps %}      
        {%- if prop.type() === 'object' %}
    ...
    private @Valid {{prop.format() | toJavaType}} {{propName | camelCase}};
            {%- else %}            
    private @Valid {{prop.type() | toJavaType}} {{propName | camelCase}}{{schema | getDiscriminatorValue(propName) | safe}};
            {%- endif %}
        {%- endif %}
    {% endfor %}
    ...
    {%- if propName | isRequired(schema.required()) and propType != 'String' %} @NotNull{% endif %}
    {%- if propName | isRequired(schema.required()) and propType == 'String' %} @NotBlank{% endif %}
    ...
    public boolean equals(Object o) {
        ...
        return {% for propName, prop in schemaCollectedProps %}{% set varName = propName | camelCase %}{% if prop.type() === 'array' %}{% set varName = propName | camelCase + 'List' %}{% endif %}
            Objects.equals(this.{{varName}}, {{schemaName | camelCase}}.{{varName}}){% if not loop.last %} &&{% else %};{% endif %}{% endfor %}
    }

    @Override
    public int hashCode() {
        return Objects.hash({% for propName, prop in schemaCollectedProps %}{{propName | camelCase}}{% if prop.type() === 'array' %}List{% endif %}{% if not loop.last %}, {% endif %}{% endfor %});
    }{% endif %}

    @Override
    public String toString() {
        return "class {{schemaName | camelCase | upperFirst}} {\n" +
        {% for propName, prop in schemaCollectedProps %}{% set varName = propName | camelCase %}{% if prop.type() === 'array' %}{% set varName = propName | camelCase + 'List' %}{% endif %}
